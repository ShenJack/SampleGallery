...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //判断当前元素的child中有没有名字匹配的，因为我们虽然不显示这些child但是仍需要匹配路由
      hasSameNameInChilds() {
        return this.$route.matched
          .slice(this.$route.matched.length-2)
          .map(item => item.name)
          .some(itemName => itemName === this.name);
      }
    },
    mounted() {
      this.$on('on-update-active-name', (name) => {
        if (this.name === name || this.hasSameNameInChilds() ) {
          this.active = true;
